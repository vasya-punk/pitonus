package ui{
	...
	public class ControlPanelBase extends View {
		...
		protected function dropDownList(properties : Object) : HGroup {
			...
			selectBox.dataSource = ["a", "b", "c", "d", "e", "f"];
			selectBox.selectItemAt(0);
			selectBox.setStyle(SelectBox.style.maxVisibleItems, 4);
			selectBox.bindProperty(
			...
			function itemSelected(item : String) : void {
				if (label) {
					label.text = (
						item + " is selected"
						+ " at " + selectBox.selectedIndex
					);
				}
			}
			
			register(selectBox, properties);

			return hgroup("gap", 4, display("id", properties["id"], selectBox), display("id", properties["labelid"], label));
		}
